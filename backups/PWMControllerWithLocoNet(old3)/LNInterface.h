...
class LNInterface
{
	private:
		...
		static FunctionNode *functions_root; 
		static FunctionNode **functions_map;
	...
	public:
		LNInterface(uint8_t tx_pin, uint16_t loco_address, uint8_t frequency = 0);
		void map(Animation *ani, uint8_t fn, bool cts);
		void mapButton(uint8_t pin_num, uint8_t fn_num);
		void printRunningSpeeds();
