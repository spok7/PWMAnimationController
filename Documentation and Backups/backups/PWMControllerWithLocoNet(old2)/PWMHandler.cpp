...
public:
	ServoHandler(uint8_t pin, uint16_t offset, Adafruit_PWMServoDriver *driver) : PWMHandler(130, 640, pin, offset, driver) {Serial.println(F("   H: New Servo!"));}
};

class RotServoHandler: public PWMHandler {
public:
	RotServoHandler(uint8_t pin, Adafruit_PWMServoDriver *driver) : PWMHandler(0, 4095, pin, 0, driver) {Serial.println(F("   H: New Rotary Servo!"));}
};

class LedHandler: public PWMHandler {
public:
	LedHandler(uint8_t pin, Adafruit_PWMServoDriver *driver) : PWMHandler(0, 4095, pin, 0, driver) {Serial.println(F("   H: New LED!"));}
};