...
RotServoHandler::RotServoHandler(uint8_t pin, Adafruit_PWMServoDriver *driver) : PWMHandler(0, 4096, pin, 0, driver) //4096 is completely off
{
	Serial.println(F("    H: Created New Rotary Servo"));
}

LEDHandler::LEDHandler(uint8_t pin, Adafruit_PWMServoDriver *driver) : PWMHandler(0, 4096, pin, 0, driver) //4096 is completely on
{
